<template>
  ...
  <div
    v-else-if="product"
    class="bg-gray-900 rounded-2xl shadow-2xl border border-gray-700 p-6 max-w-5xl mx-auto mt-10"
  >
      ...
      <div class="flex flex-col justify-between w-full md:w-1/2 text-gray-100">
        ...
        <div class="flex justify-between items-center mt-6">
          <div class="flex items-end">
            ...
            <span v-if="product.originalPrice" class="text-gray-500 text-sm line-through ml-3 mb-1">
    ...
    <div>
      <p class="text-gray-400 mb-4 pt-4 text-lg text-justify">
        {{
          showFull
            ? product.description || "Descripción no disponible"
            : product.description?.slice(0, 400) || "Descripción no disponible"
        }}
        <span
          v-if="(product.description?.length || 0) > 150"
          @click="showFull = !showFull"
          class="text-orange-400 cursor-pointer ml-2 hover:underline"
        >
  ...
  <div class="flex justify-center items-center mt-6 space-x-1 text-yellow-400 text-2xl">
    <el-icon v-for="n in 5" :key="n">
      <component :is="StarFilled" :class="n <= averageRating ? 'opacity-100' : 'opacity-30'" />
    </el-icon>
  </div>
  ...
  <div class="max-w-3xl mx-auto mt-10 text-white">
      ...
      >
        <div class="flex items-center mb-2">
          ...
          <div class="flex ml-4 text-yellow-400 text-lg">
            <el-icon v-for="n in 5" :key="n">
              <Star
                :class="{ 'opacity-100': n <= review.rating, 'opacity-20': n > review.rating }"
              />
            </el-icon>
          </div>
        </div>
    ...
    <form
      ...
      class="mt-6 space-y-4 bg-gray-800 p-4 pb-10rounded-lg border border-gray-700"
    >
...
const reviews = ref([{ rating: 1 }, { rating: 2 }, { rating: 3 }, { rating: 4 }, { rating: 5 }]);
